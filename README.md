...
`.env` を開いて、Railway の PostgreSQL の接続情報を設定する：

```
PORT=3003
DATABASE_URL=（Railway ダッシュボードの DATABASE_PUBLIC_URL の値）
```

> [!NOTE]
> `DATABASE_PUBLIC_URL` はローカル開発用の外部接続 URL です。Railway の PostgreSQL サービスの「Variables」タブから取得できます。接続情報はチームメンバーに Slack 等で共有してください。
...
`http://localhost:3003/health` にアクセスして `{"status":"ok","db":"connected"}` が返れば成功です。
